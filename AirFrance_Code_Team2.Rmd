...
author: "Keyword Count and Match Type"
output:
...
#import dataset 
airfrance <- read_excel("Air France R team data.xlsx")
...
```
As seen in the output the number of missing values is 0.

### Massage the Data

1. Create a new column to label Overture as Yahoo, and call it: 'New Publisher Name'
```{r,  warning = FALSE}
...
2. Create a new column to group match types as 'Broad' or 'Exact':
...
3. Create a new column in the dataset for Return on Asset (ROA):
```{r, warning = FALSE}
...
```
3. Alternative, Creating ROA variable 
```{r}
##################################################################################################################
...
```
##### Interpretation: 
Yahoo is the best search engine publisher for both the US and Global air market. However, to best utilize Yahoo, we need to specify the match type used to search optimization. In the US, Exact types will work best - bringing in 221.67 dollars, the highest ROA on average - in comparison to other US publishers. While in the Global market, Broad types will work best - reeling in 151.56 dollars, also the highest ROA on average when comparing to other global publishers. 
...
##### Interpretation: 
Based on the boxplot, we can obviously see how different keyword match type brings in different ROA. The ranking for each region is as follow (from highest to lowest ROA):
...
```
Business in this case is an ROA that is greater than the average ROA across all the data

#### More pivot tables using a different function in R
```{r message=FALSE, warning=FALSE}
##################################################################################################################
###Comparisons of publisher 
#################################################################################################################
...
```
### Insights from publisher v  Match type analysis

1. A standardized strategy on match type would not be optimal 
2. Match types tailored to each publisher would seem better 
3. We would require more info:
   -Customer segments 
   -Ages
   -Language backgrounds
   -Education 
   -Browser they are accessing from 
4. Further analysis in our study will show what other trackers we can use to help maximize the Return on Assets invested in each publisher

### Click charges,Total Cost, Total Volume of bookings
```{r}
...
Both the pivot table and the boxplot show that to maximize return on investment: Media Contacts must tailor strategy based on search engine publisher. A uniformed strategy will not work. Media Contacts can also improve match types by: 

(1) Implementing 'Phrase Match Type': This is a modified version of 'exact match' by honing in the intended audience better than broad match type but not as restrictive as exact match.

For example: 
1. Exact Type: [airfrance] --> audience would need to search exactly 'airfrance' for ad to show
2. Phrase Match Type: 'air france' --> if audience searched 'air france travel' or 'air france cost', the ad will still show 

(2) Implementing 'Modified Broad Match Type': The name is self-explanatory; it keep keywords flexible while allowing you to have more control over who sees your ad and cut down irrelevant traffic.

For example:
1. Broad Type: air discount france ticket --> publishers will show everyone who is looking for air, discount, tickets, and France, but not necessarily in that combination.
2. Modified Broad Type: air+discount+france+ticket --> this will tell publishers to only show your ad if people are looking for discounted tickets to France 

## KEYWORD COUNT --> CONCLUSION: TOP 10 KEYWORDS TO USE 
...
```
#### CONCLUSION & RECOMMENDATION:
More frequently used words; meaning they must be popular/driven in the most awareness and/or conversion. Applying what we know about Match Types, we can use these top 10 keywords as stand-alone (Exact Type or Broad Type) or put them in combos (Modified Broad Type or Phrase Type) to optimize audience reach. 
